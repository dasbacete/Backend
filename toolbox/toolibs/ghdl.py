...
class ghdl_handler ( handler.handler, pfile.pfile ):
    ...
    def analyze ( self, pfile_path=None, cmdl_flags=None ):
        srcs = None
        local_pfile = None
        if pfile_path:
        ...
        #process_flags
        run_flags = process_flags([self.stageXflags['analyze'], cmdl_flags])
        
        flags = f"-a {run_flags} {filelist}"
        return self.launch(flags)

    def elaborate ( self, top=None, cmdl_flags=None ):
        top_unit = self.get_top()
        if top is not None:
            top_unit = top

        #process_flags
        run_flags = process_flags([self.stageXflags['elaborate'], cmdl_flags])
        
        flags = f"-e {run_flags} {top_unit}"
        return self.launch(flags)

    def simulate ( self, top=None, cmdl_flags=None):
        top_unit = self.get_top()
        if top is not None:
            top_unit = top
        sim_flags = []
        if self.sim:
            if top_unit in self.sim:
                if 'sim_flags' in self.sim[top_unit]:
                    sim_flags += self.sim[top_unit]['sim_flags']
                if 'cosim' in self.sim[top_unit]:
                    sim_flags.append(f"--vpi=$(cocotb-config --lib-name-path vpi ghdl)")
                    ...
                    os.environ['TESTCASE']      = self.sim[top_unit]['test']
        
        #process_flags
        run_flags = process_flags([self.stageXflags['simulate'], sim_flags , cmdl_flags])
        
        flags = f"-r {top_unit} {run_flags}"
        return self.launch(flags)
